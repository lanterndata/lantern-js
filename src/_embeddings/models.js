const TextEmbeddingModelsValue = {
...
const ImageEmbeddingModelsValue = { CLIP_VIT_B_32_VISUAL: 'clip/ViT-B-32-visual' };

function getTextEmbeddingModelName(modelKey) {
  if (!TextEmbeddingModelsValue[modelKey]) {
    throw new Error('Invalid text embedding model');
  }

  return TextEmbeddingModelsValue[modelKey];
}
...
function getImageEmbeddingModelName(modelKey) {
  ...
  return ImageEmbeddingModelsValue[modelKey];
}
